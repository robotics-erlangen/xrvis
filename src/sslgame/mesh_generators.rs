...
pub fn field_mesh(
    ...
    let mut goal_yellow_mesh = BevyMesh3d::new();

    let goal_y_bottom = goal_yellow_mesh.insert_loop(
        [
            // Outer
            Vec3::new(-border_x, 0.0, -goal_y - GOAL_WALL_HALF),
            Vec3::new(-field_x, 0.0, -goal_y - GOAL_WALL_HALF),
            Vec3::new(-field_x, 0.0, goal_y + GOAL_WALL_HALF),
            Vec3::new(-border_x, 0.0, goal_y + GOAL_WALL_HALF),
            // Inner
            Vec3::new(-border_x, 0.0, goal_y - GOAL_WALL_HALF),
            Vec3::new(-field_x + GOAL_WALL, 0.0, goal_y - GOAL_WALL_HALF),
            Vec3::new(-field_x + GOAL_WALL, 0.0, -goal_y + GOAL_WALL_HALF),
            Vec3::new(-border_x, 0.0, -goal_y + GOAL_WALL_HALF),
        ]
        .map(BevyVertexPayload3d::from_pos),
    );
    let goal_y_bottom = goal_yellow_mesh.edge(goal_y_bottom).twin_id();
    let goal_y_top = goal_yellow_mesh.loft_polygon(
        goal_y_bottom,
        2,
        2,
        [
            // Outer
            Vec3::new(-border_x, WALL_HEIGHT, -goal_y - GOAL_WALL_HALF),
            Vec3::new(-field_x, WALL_HEIGHT, -goal_y - GOAL_WALL_HALF),
            Vec3::new(-field_x, WALL_HEIGHT, goal_y + GOAL_WALL_HALF),
            Vec3::new(-border_x, WALL_HEIGHT, goal_y + GOAL_WALL_HALF),
            // Inner
            Vec3::new(-border_x, WALL_HEIGHT, goal_y - GOAL_WALL_HALF),
            Vec3::new(-field_x + GOAL_WALL, WALL_HEIGHT, goal_y - GOAL_WALL_HALF),
            Vec3::new(-field_x + GOAL_WALL, WALL_HEIGHT, -goal_y + GOAL_WALL_HALF),
            Vec3::new(-border_x, WALL_HEIGHT, -goal_y + GOAL_WALL_HALF),
        ]
        .map(BevyVertexPayload3d::from_pos),
    );
    goal_yellow_mesh.close_hole_default(goal_y_top);

    let mut goal_blue_mesh = BevyMesh3d::new();

    let goal_b_bottom = goal_blue_mesh.insert_loop(
        [
            // Outer
            Vec3::new(border_x, 0.0, goal_y + GOAL_WALL_HALF),
            Vec3::new(field_x, 0.0, goal_y + GOAL_WALL_HALF),
    ...
    let goal_b_bottom = goal_blue_mesh.edge(goal_b_bottom).twin_id();
    let goal_b_top = goal_blue_mesh.loft_polygon(
        goal_b_bottom,
        2,
        2,
        [
    ...
    goal_blue_mesh.close_hole_default(goal_b_top);
    ...
    line_mesh.insert_polygon(
        [
            Vec3::new(-LINE_HALF, 0.0001, border_y),
            Vec3::new(-LINE_HALF, 0.0001, -border_y),
            Vec3::new(LINE_HALF, 0.0001, -border_y),
            Vec3::new(LINE_HALF, 0.0001, border_y),
        ]
    ...
    let (line_defense_y_inner, _) = line_mesh.insert_path(
        [
            Vec3::new(-border_x, 0.0001, -defense_y + LINE_HALF),
    ...
    world.spawn_batch([
        ...
        (
            Transform::default(),
            Mesh3d(mesh_assets.add(goal_yellow_mesh.to_bevy(RenderAssetUsages::default()))),
            MeshMaterial3d(material_assets.add(goal_y_mat)),
        ),
        (
            Transform::default(),
            Mesh3d(mesh_assets.add(goal_blue_mesh.to_bevy(RenderAssetUsages::default()))),
...
pub fn polygon_visualization_mesh(vertices: &[Vec2]) -> Mesh {
    ...
    // FIXME: Polygon visualizations sometimes point down
    ...
    mesh.insert_polygon(
        vertices
            .iter()
            .map(|p| BevyVertexPayload3d::from_pos(Vec3::new(p.x, 0.0, p.y))),
    );
